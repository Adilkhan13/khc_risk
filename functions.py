...
def upload_new_data(new_file_path)->pd.DataFrame:
    ...
    if len(sheets) >1:
        ...
        try:
            df['Кол-во дней просрочки по ОД'] = df['Кол-во дней просрочки по ОД'].astype(float)
            df['Кол-во дней просрочки по НВ'] = df['Кол-во дней просрочки по НВ'].astype(float)
            df['Кол-во дней просрочки по гарантийному взносу'] = df['Кол-во дней просрочки по гарантийному взносу'].astype(float)
        except:
        ...
        pd.concat([pd.read_csv('./datasets/loggs_k.csv'),loggs]).to_csv('./datasets/loggs_k.csv', index = False)
        return df


def add_merge(path)->None:
    """
    ...
    if df_add_or_replace['Oтчетная дата'].iloc[0] > df_main['Oтчетная дата'].max():
        ...
        df_main = df_main[df_main['Oтчетная дата'] != df_add_or_replace['Oтчетная дата'].iloc[0]]
        df = pd.concat([df_main,df_add_or_replace],axis = 0)
        df.to_csv(f'./datasets/main_data/full_k_{date.today()}.csv',index = False)
...
if __name__ == "__main__":
    ...
    df = load_data()
    ids = ids(df)
    vintage(df,ids).to_excel('./datasets/vintage/vintage_кик_' +str(date.today())+ '.xlsx')
#